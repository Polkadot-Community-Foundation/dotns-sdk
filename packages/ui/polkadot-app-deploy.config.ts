...
// `packages/ui/dist` → `packages/ui/`) and reads the default export to publish
// the product manifest (display name, description, icon) alongside the content
// upload. A file named anything else is silently ignored — manifest publish
// skipped, no error.
//
...
//
// This file lives OUTSIDE the tsconfig `include` globs (which cover `src/**`
// only), so it does not affect typecheck or the Vite build.
...
const domain = process.env?.APP_DOTNS_DOMAIN ?? "dotns";
const label = domain.toLowerCase().replace(/\.dot$/, "");

export default defineConfig({
  domain: `${label}.dot`,
  displayName: "DotNS",
  description: "Register and manage .dot names on Polkadot.",
